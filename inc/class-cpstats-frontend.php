<?php
/**
 * CPStats: CPStats_Frontend class
 *
 ...
 * @since 1.4.0
 */
class CPStats_Frontend extends CPStats {
	...
	public static function track_visit( $is_snippet = false ) {
		...
		if ( self::is_javascript_tracking_enabled() ) {
			...
			if ( isset( $_REQUEST['cpstats_target'] ) ) {
				$target = filter_var( wp_unslash( $_REQUEST['cpstats_target'] ), FILTER_SANITIZE_URL );
			}
			if ( isset( $_REQUEST['cpstats_referrer'] ) ) {
				$referrer = filter_var( wp_unslash( $_REQUEST['cpstats_referrer'] ), FILTER_SANITIZE_URL );
			}
		} else {
		 ...
		 */
		do_action( 'cpstats__visit_saved', $data, $wpdb->insert_id );
	...
	public static function track_visit_ajax() {
		...
		// Check AJAX referrer.
		if ( CPStats::TRACKING_METHOD_JAVASCRIPT_WITH_NONCE_CHECK === self::$_options['snippet'] ) {
			check_ajax_referer( 'cpstats_track' );
		}
	...
	private static function _skip_tracking() {
		...
		if ( function_exists( 'apply_filters_deprecated' ) ) {
			apply_filters_deprecated( 'cpstats_skip_tracking', array( '' ), '1.5.0', 'cpstats__skip_tracking' );
		}
		// Skip tracking via Hook.
		$skip_hook = apply_filters( 'cpstats__skip_tracking', null );
		if ( null !== $skip_hook ) {
			return $skip_hook;
		}
	...
	public static function wp_footer() {
		...
		wp_localize_script(
			...
			array(
				...
				'nonce' => wp_create_nonce( 'cpstats_track' ),
			)
		);
	}
	...
	public static function amp_analytics_entries( $analytics_entries ) {
		...
		if ( self::is_javascript_tracking_enabled() ) {
			$analytics_entries['cpstats'] = array(
				'type'   => '',
	...
	private static function make_amp_config() {
		return array(
			...
			'extraUrlParams' => array(
				...
				'_ajax_nonce'      => wp_create_nonce( 'cpstats_track' ),
				'cpstats_referrer' => '${documentReferrer}',
				'cpstats_target'   => '${canonicalPath}amp/',
			),
			'triggers'       => array(
				'trackPageview' => array(
					'on'      => 'visible',
