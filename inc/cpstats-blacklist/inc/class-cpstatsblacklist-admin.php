<?php
/**
 * CPStats Filter: CPStatsBlacklist_Admin class
 *
...
/**
 * CPStats Filter admin configuration.
 *
 * @since   1.0.0
 */
class CPStatsBlacklist_Admin extends CPStatsBlacklist {
	...
	public static function init() {
		// Add actions.
		add_action( 'wpmu_new_blog', array( 'CPStatsBlacklist_System', 'install_site' ) );
		add_action( 'delete_blog', array( 'CPStatsBlacklist_System', 'uninstall_site' ) );
		add_filter( 'plugin_row_meta', array( 'CPStatsBlacklist_Admin', 'plugin_meta_link' ), 10, 2 );

		if ( self::$multisite ) {
			add_action( 'network_admin_menu', array( 'CPStatsBlacklist_Admin', 'add_menu_page' ) );
			add_filter(
			...
			add_action( 'admin_menu', array( 'CPStatsBlacklist_Admin', 'add_menu_page' ) );
			add_filter( 'plugin_action_links', array( 'CPStatsBlacklist_Admin', 'plugin_actions_links' ), 10, 2 );
		}
	}
	...
	public static function update_options( $options = null ) {
		if ( isset( $options ) && current_user_can( 'manage_options' ) ) {
			...
			if ( self::$multisite ) {
				update_site_option( 'cpstats-blacklist', $options );
			} else {
				update_option( 'cpstats-blacklist', $options );
			}
		}
	...
	/**
	 * Include the CPStats-Blacklist settings page.
	 *
	 * @since 1.0.0
	 */
	...
	public static function plugin_meta_link( $links, $file ) {
		if ( CPSTATSBLACKLIST_BASE === $file ) {
			$links[] = '<a href="https://github.com/stklcode/cpstats-blacklist">GitHub</a>';
		}
	...
	public static function cleanup_database() {
		...
		if ( ! empty( $referer_regexp ) || ! empty( $target_regexp ) ) {
			...
			if ( ! empty( $referer_regexp ) ) {
				$wpdb->query(
					$wpdb->prepare(
						"DELETE FROM `$wpdb->cpstats` WHERE "
			...
			// Optimize DB.
			$wpdb->query( "OPTIMIZE TABLE `$wpdb->cpstats`" );

			// Delete transient cpstats data.
	...
	private static function sanitize_regex( $expressions ) {
		return array_filter(
			...
			function ( $re ) {
				...
				return false === @preg_match( CPStatsBlacklist::regex( $re, false ), null );
			}
		);
	}
}
