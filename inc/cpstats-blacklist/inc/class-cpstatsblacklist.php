<?php
/**
 * CPStats Filter: CPStatsBlacklist class
 *
...
class CPStatsBlacklist {
	...
	public static function init() {
		...
		// Add Filter to cpstats hook if enabled.
		if ( 0 !== self::$options['referer']['active'] ||
			0 !== self::$options['target']['active'] ||
			0 !== self::$options['ip']['active'] ||
			0 !== self::$options['ua']['active'] ) {
			add_filter( 'cpstats__skip_tracking', array( 'CPStatsBlacklist', 'apply_blacklist_filter' ) );
		}

		// CPStats uses WP AJAX as of 1.7, so we need to reach this point. But there are no further admin/cron actions.
		...
		// Admin only filters.
		if ( is_admin() ) {
			CPStatsBlacklist_Admin::init();
		}

		// CronJob to clean up database.
		if ( defined( 'DOING_CRON' ) && DOING_CRON &&
			( 1 === self::$options['referer']['cron'] || 1 === self::$options['target']['cron'] ) ) {
			add_action( 'cpstats_cleanup', array( 'CPStatsBlacklist_Admin', 'cleanup_database' ) );
		}
	}
	...
	public static function update_options( $options = null ) {
		if ( self::$multisite ) {
			$o = get_site_option( 'cpstats-blacklist' );
		} else {
			$o = get_option( 'cpstats-blacklist' );
		}
