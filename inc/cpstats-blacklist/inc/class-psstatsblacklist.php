<?php
/**
 * PSStats Filter: PSStatsBlacklist class
 *
...
class PSStatsBlacklist {
	...
	public static function init() {
		...
		self::$multisite = ( is_multisite() && array_key_exists( CPSTATSBLACKLIST_BASE, (array) get_site_option( 'active_sitewide_plugins' ) ) );

		// Plugin options.
		self::update_options();

		// Add Filter to psstats hook if enabled.
		if ( 0 !== self::$options['referer']['active'] ||
			0 !== self::$options['target']['active'] ||
			0 !== self::$options['ip']['active'] ||
			0 !== self::$options['ua']['active'] ) {
			add_filter( 'psstats__skip_tracking', array( 'PSStatsBlacklist', 'apply_blacklist_filter' ) );
		}

		// PSStats uses WP AJAX as of 1.7, so we need to reach this point. But there are no further admin/cron actions.
		...
		// Admin only filters.
		if ( is_admin() ) {
			PSStatsBlacklist_Admin::init();
		}

		// CronJob to clean up database.
		if ( defined( 'DOING_CRON' ) && DOING_CRON &&
			( 1 === self::$options['referer']['cron'] || 1 === self::$options['target']['cron'] ) ) {
			add_action( 'psstats_cleanup', array( 'PSStatsBlacklist_Admin', 'cleanup_database' ) );
		}
	}
	...
	public static function update_options( $options = null ) {
		if ( self::$multisite ) {
			$o = get_site_option( 'psstats-blacklist' );
		} else {
			$o = get_option( 'psstats-blacklist' );
		}
