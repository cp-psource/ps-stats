...
function eefcpstats_echo_export_button( $filename, $button_id = 'csv-export', $table_id = 'table-data' ) {
	...
	jQuery(document).ready(function () {
		jQuery("#<?php echo esc_html( $button_id ); ?>").click(function () {
			eefcpstatsTableToCsv.apply(this, [jQuery('#<?php echo esc_html( $table_id ); ?>'), '<?php echo esc_html( $filename ); ?>.csv']);
		});
	});
	</script>
	<?php
 ...
 */
function eefcpstats_echo_date_selection( $valid_start, $start, $valid_end, $end ) {
	if ( ! $valid_start ) {
		$start = '';
	}
	if ( ! $valid_end ) {
...
function eefcpstats_echo_post_selection( $selected_post ) {
	...
	<datalist id="posts">
		<?php foreach ( $posts as $post ) { ?>
			<option value="<?php echo esc_html( $post['target'] ); ?>"><?php echo esc_html( eefcpstats_get_post_title_from_url( $post['target'] ) ); ?></option>
		<?php } ?>
	</datalist>
	<?php
}
 ...
 */
function eefcpstats_echo_tab_class( $is_active_tab ) {
	echo 'nav-tab';
	if ( $is_active_tab ) {
		echo ' nav-tab-active';
	}
 ...
 */
function eefcpstats_is_valid_date_string( $date ) {
	if ( 10 !== strlen( $date ) ) {
		return false;
	}
 ...
 */
function eefcpstats_get_date_period_string( $start, $end, $show, $formatted_dates = false ) {
	if ( $show ) {
		if ( $formatted_dates ) {
 ...
 */
function eefcpstats_get_filename( $export_name ) {
 ...
 */
function eefcpstats_echo_number( $string ) {
	$number = (int) $string;
	if ( $number < 0 ) {
		echo '&mdash;';
	} else {
 ...
 */
function eefcpstats_echo_percentage( $number, $decimals = 2 ) {
 ...
 */
function eefcpstats_get_month_name( $month_as_int ) {
	$month_as_int = (int) $month_as_int;
	if ( in_array( $month_as_int, eefcpstats_get_months(), true ) ) {
 ...
 */
function eefcpstats_get_month_year_name( $month ) {
 ...
 */
function eefcpstats_get_number_for_csv( $string ) {
	$number = (int) $string;
	if ( $number < 0 ) {
		return '-';
	}
 ...
 */
function eefcpstats_get_post_type_from_url( $url ) {
	$post_id = url_to_postid( $url );
 ...
 */
function eefcpstats_get_post_type_name_from_url( $url ) {
	$post_type = eefcpstats_get_post_type_from_url( $url );
 ...
 */
function eefcpstats_get_post_types() {
	$types_args = array(
		'public' => true,
		'_builtin' => false,
	);
 ...
 */
function eefcpstats_echo_chart_container( $id, $title, $subtitle = '', $legend = [] ) {
	?>
