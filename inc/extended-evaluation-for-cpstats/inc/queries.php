 ...
 */
function eefcpstats_get_days( $month = 1, $year = 0 ) {
	if ( 2 === $month ) {
 ...
 */
function eefcpstats_get_months() {
	return range( 1, 12 );
}

/**
 * Returns the years CPStats has collected data for in descending order.
 *
 * @return array an array of integers (e. g. 2016, 2015)
 */
function eefcpstats_get_years() {
	global $wpdb;
 ...
 */
function eefcpstats_get_views_for_all_days( $post_url = '' ) {
	global $wpdb;
	if ( '' === $post_url ) {
		// For all posts.
		$results = $wpdb->get_results(
 ...
 */
function eefcpstats_get_daily_views( $views_for_all_days, $year, $month, $day ) {
 ...
 */
function eefcpstats_get_views_for_all_months( $post_url = '' ) {
	global $wpdb;
	if ( '' === $post_url ) {
		// For all posts.
		$results = $wpdb->get_results(
 ...
 */
function eefcpstats_get_monthly_views( $views_for_all_months, $year, $month ) {
	$date = sprintf(
		'%s-%s',
		str_pad( $year, 4, '0', STR_PAD_LEFT ),
		str_pad( $month, 2, '0', STR_PAD_LEFT )
 ...
 */
function eefcpstats_get_average_daily_views_of_month( $views_for_all_months, $year, $month ) {
	$views_in_month = eefcpstats_get_monthly_views( $views_for_all_months, $year, $month );
	$days_in_month  = eefcpstats_is_current_month( $year, $month )
		? (int) gmdate( 'd' )
		: count( eefcpstats_get_days( $month, $year ) );
 ...
 */
function eefcpstats_get_daily_views_of_month( $views_for_all_days, $year, $month ) {
	if ( eefcpstats_is_current_month( $year, $month ) ) {
		$days = range( 1, (int) gmdate( 'd' ) );
	} else {
		$days = eefcpstats_get_days( $month, $year );
	}

	$views = [];
	foreach ( $days as $day ) {
		$views[] = (int) eefcpstats_get_daily_views( $views_for_all_days, $year, $month, $day );
	}
 ...
 */
function eefcpstats_is_current_month( $year, $month ) {
 ...
 */
function eefcpstats_get_views_for_all_years( $post_url = '' ) {
	global $wpdb;
	if ( '' === $post_url ) {
		// For all posts.
		$results = $wpdb->get_results(
 ...
 */
function eefcpstats_get_yearly_views( $views_for_all_years, $year ) {
 ...
 */
function eefcpstats_get_views_of_most_popular_posts( $start = '', $end = '' ) {
	global $wpdb;
	if ( '' === $start && '' === $end ) {
 ...
 */
function eefcpstats_get_views_of_post( $url, $start = '', $end = '' ) {
	global $wpdb;
	if ( '' === $start && '' === $end ) {
		$results = $wpdb->get_results(
			$wpdb->prepare(
 ...
 */
function eefcpstats_get_views_for_all_referrers( $post_url = '', $start = '', $end = '' ) {
	global $wpdb;
	if ( '' === $post_url ) {
		// For all posts.
		if ( '' === $start && '' === $end ) {
 ...
 */
function eefcpstats_get_post_urls() {
	global $wpdb;
