 ...
 */
function eefpsstats_echo_export_button( $filename, $button_id = 'csv-export', $table_id = 'table-data' ) {
	?>
	<a class="page-title-action" href="#" id="<?php echo esc_html( $button_id ); ?>" role="button"><?php esc_html_e( 'Export', 'psstats' ); ?></a>
	<script type='text/javascript'>
	jQuery(document).ready(function () {
		jQuery("#<?php echo esc_html( $button_id ); ?>").click(function () {
			eefpsstatsTableToCsv.apply(this, [jQuery('#<?php echo esc_html( $table_id ); ?>'), '<?php echo esc_html( $filename ); ?>.csv']);
		});
	});
	</script>
	<?php
 ...
 */
function eefpsstats_echo_date_selection( $valid_start, $start, $valid_end, $end ) {
	if ( ! $valid_start ) {
		$start = '';
	}
	if ( ! $valid_end ) {
	...
	?>
	<fieldset>
		<legend><?php esc_html_e( 'Restrict date period: Please enter start and end date in the YYYY-MM-DD format', 'psstats' ); ?></legend>
		<label for="dateRange"><?php esc_html_e( 'Date range', 'psstats' ); ?></label>
		<select id="dateRange" onchange="eefpsstatsSelectDateRange()">
			<option value="default"><?php esc_html_e( 'default (all the time)', 'psstats' ); ?></option>
			<option value="lastYear"><?php esc_html_e( 'last year', 'psstats' ); ?></option>
			<option value="lastWeek"><?php esc_html_e( 'last week', 'psstats' ); ?></option>
			<option value="yesterday"><?php esc_html_e( 'yesterday', 'psstats' ); ?></option>
			<option value="today"><?php esc_html_e( 'today', 'psstats' ); ?></option>
			<option value="thisWeek"><?php esc_html_e( 'this week', 'psstats' ); ?></option>
			<option value="last28days"><?php esc_html_e( 'last 28 days', 'psstats' ); ?></option>
			<option value="lastMonth"><?php esc_html_e( 'last month', 'psstats' ); ?></option>
			<option value="thisMonth"><?php esc_html_e( 'this month', 'psstats' ); ?></option>
			<option value="thisYear"><?php esc_html_e( 'this year', 'psstats' ); ?></option>
			<option value="1stQuarter"><?php esc_html_e( '1st quarter', 'psstats' ); ?></option>
			<option value="2ndQuarter"><?php esc_html_e( '2nd quarter', 'psstats' ); ?></option>
			<option value="3rdQuarter"><?php esc_html_e( '3rd quarter', 'psstats' ); ?></option>
			<option value="4thQuarter"><?php esc_html_e( '4th quarter', 'psstats' ); ?></option>
			<option value="custom"><?php esc_html_e( 'custom', 'psstats' ); ?></option>
		</select>
		<label for="start"><?php esc_html_e( 'Start date', 'psstats' ); ?></label>
		<input id="start" name="start" type="date" value="<?php echo esc_html( $start ); ?>" oninput="eefpsstatsDateRangeChange()"
			   pattern="(?:19|20)[0-9]{2}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-9])|(?:(?!02)(?:0[1-9]|1[0-2])-(?:30))|(?:(?:0[13578]|1[02])-31))">
		<label for="end"><?php esc_html_e( 'End date', 'psstats' ); ?></label>
		<input id="end" name="end" type="date" value="<?php echo esc_html( $end ); ?>" oninput="eefpsstatsDateRangeChange()"
			   pattern="(?:19|20)[0-9]{2}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-9])|(?:(?!02)(?:0[1-9]|1[0-2])-(?:30))|(?:(?:0[13578]|1[02])-31))">
		<button type="submit" class="button-secondary"><?php esc_html_e( 'Select date period', 'psstats' ); ?></button>
	</fieldset>
	<?php
}
 ...
 */
function eefpsstats_echo_post_selection( $selected_post ) {
	$posts = eefpsstats_get_post_urls();
	?>
	<label for="post"><?php esc_html_e( 'Post/Page', 'psstats' ); ?></label>
	<input id="post" name="post" type="text" list="posts" value="<?php echo esc_attr( $selected_post ); ?>">
	<datalist id="posts">
		<?php foreach ( $posts as $post ) { ?>
			<option value="<?php echo esc_html( $post['target'] ); ?>"><?php echo esc_html( eefpsstats_get_post_title_from_url( $post['target'] ) ); ?></option>
		<?php } ?>
	</datalist>
	<?php
}
 ...
 */
function eefpsstats_echo_tab_class( $is_active_tab ) {
	echo 'nav-tab';
	if ( $is_active_tab ) {
		echo ' nav-tab-active';
	}
 ...
 */
function eefpsstats_is_valid_date_string( $date ) {
	if ( 10 !== strlen( $date ) ) {
		return false;
	}
 ...
 */
function eefpsstats_get_date_period_string( $start, $end, $show, $formatted_dates = false ) {
	if ( $show ) {
		if ( $formatted_dates ) {
 ...
 */
function eefpsstats_get_filename( $export_name ) {
 ...
 */
function eefpsstats_echo_number( $string ) {
	$number = (int) $string;
	if ( $number < 0 ) {
		echo '&mdash;';
	} else {
 ...
 */
function eefpsstats_echo_percentage( $number, $decimals = 2 ) {
 ...
 */
function eefpsstats_get_month_name( $month_as_int ) {
	$month_as_int = (int) $month_as_int;
	if ( in_array( $month_as_int, eefpsstats_get_months(), true ) ) {
 ...
 */
function eefpsstats_get_month_year_name( $month ) {
 ...
 */
function eefpsstats_get_number_for_csv( $string ) {
	$number = (int) $string;
	if ( $number < 0 ) {
		return '-';
	}
 ...
 */
function eefpsstats_get_post_title_from_url( $url ) {
	if ( '' === $url ) {
		return __( 'all posts', 'psstats' );
	}
	if ( '/' === $url ) {
		return __( 'Home Page', 'psstats' );
	}
 ...
 */
function eefpsstats_get_post_type_from_url( $url ) {
	$post_id = url_to_postid( $url );
 ...
 */
function eefpsstats_get_post_type_name_from_url( $url ) {
	$post_type = eefpsstats_get_post_type_from_url( $url );
 ...
 */
function eefpsstats_get_post_type_name_and_title_from_url( $url ) {
	return sprintf(
		/* translators: 1: post type 2: post title or URL. */
		__( 'for %1$s %2$s', 'psstats' ),
		eefpsstats_get_post_type_name_from_url( $url ),
		eefpsstats_get_post_title_from_url( $url )
	);
}
 ...
 */
function eefpsstats_get_post_types() {
	$types_args = array(
		'public' => true,
		'_builtin' => false,
	);
 ...
 */
function eefpsstats_echo_chart_container( $id, $title, $subtitle = '', $legend = [] ) {
	?>
