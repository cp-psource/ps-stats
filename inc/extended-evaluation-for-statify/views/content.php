...
if ( 'popular' === $selected_post_type ) {
	...
	if ( $valid_start && $valid_end ) {
		$views_per_post = eefcpstats_get_views_of_most_popular_posts( $start, $end );
	} else {
		$views_per_post = eefcpstats_get_views_of_most_popular_posts();
	}
	...
	<section>
		...
		foreach ( $views_per_post_for_diagram as $post ) {
			$legend[] = eefcpstats_get_post_title_from_url( $post['url'] );
		}
			...
			eefcpstatsColumnChart(
				...
				[
					...
					foreach ( $views_per_post_for_diagram as $index => $post ) {
						echo "['"
							 . esc_js( $index + 1 . '. ' . eefcpstats_get_post_title_from_url( $post['url'] ) )
							 . "',"
							 . esc_js( $post['count'] )
							 . '],';
					}
			...
			<tbody>
				...
				<tr>
					<td><a href="<?php echo esc_url( home_url( $post['url'] ) ); ?>" target="_blank">
							<?php echo esc_html( eefcpstats_get_post_title_from_url( $post['url'] ) ); ?>
						</a></td>
					<td><?php echo esc_url( $post['url'] ); ?></td>
					<td><?php echo esc_html( eefcpstats_get_post_type_name_from_url( $post['url'] ) ); ?></td>
					<td class="right"><?php eefcpstats_echo_number( $post['count'] ); ?></td>
					<td class="right"><?php eefcpstats_echo_percentage( $post['count'] / $total ); ?></td>
				</tr>
				<?php } ?>
			</tbody>
			<tfoot>
	...
	<section>
		...
		<table id="table-data" class="wp-list-table widefat striped">
			...
			while ( $query->have_posts() ) :
				$query->the_post();
				if ( $valid_start && $valid_end ) {
					$views = eefcpstats_get_views_of_post(
						str_replace( home_url(), '', get_permalink() ),
						$start,
						$end
					);
				} else {
					$views = eefcpstats_get_views_of_post( str_replace( home_url(), '', get_permalink() ) );
				}
				?>
				<tr>
					<td><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></td>
					<td><?php echo esc_url( wp_make_link_relative( get_the_permalink() ) ); ?></td>
					<td class="right"><?php eefcpstats_echo_number( $views ); ?></td>
				</tr>
			<?php endwhile; ?>
			</tbody>
		</table>
